...
do $$
begin
  if not exists (select 1 from auth.users where id='236dabb3-f962-4c27-b6d0-db93699a2643'::uuid) then
  ...
  if exists (select 1 from information_schema.tables where table_schema='public'
    and table_name in ('companies','profiles','roles','permissions','role_permissions','employees','projects','project_members','tasks')) then
    raise exception 'ABBRUCH: mindestens eine Pilot-Tabelle existiert bereits; Baseline erneut prüfen.';
  end if;
end $$;

create table public.companies (id uuid primary key, name text not null, created_at timestamptz not null default now());
insert into public.companies values ('02165c75-59fa-4aa5-bb45-b2f3c4145761','IB Raumkonzept GmbH',now());
...
insert into public.roles values ('mitarbeiter','Mitarbeiter'),('bauleiter','Bauleiter'),('geschaeftsfuehrer','Geschäftsführer');
create table public.permissions (id text primary key);
insert into public.permissions values ('tasks.view'),('tasks.create'),('tasks.edit'),('tasks.delete');
...
insert into public.role_permissions values
 ...
 ('geschaeftsfuehrer','tasks.view'),('geschaeftsfuehrer','tasks.create'),('geschaeftsfuehrer','tasks.edit'),('geschaeftsfuehrer','tasks.delete');
...
create table public.employees (
  ...
  name text not null, rolle text not null default 'mitarbeiter' references public.roles(id),
  status text not null default 'aktiv' check(status in ('aktiv','inaktiv')),
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create unique index idx_employees_legacy_unique on public.employees(company_id,legacy_id) where legacy_id is not null;
create index idx_employees_company on public.employees(company_id,status);
insert into public.profiles(id,company_id) values ('236dabb3-f962-4c27-b6d0-db93699a2643','02165c75-59fa-4aa5-bb45-b2f3c4145761');
insert into public.employees(legacy_id,company_id,auth_user_id,name,rolle,status) values
 ('mrwbp45ien9rg83','02165c75-59fa-4aa5-bb45-b2f3c4145761','236dabb3-f962-4c27-b6d0-db93699a2643','Berat Shala','geschaeftsfuehrer','aktiv');
...
create unique index idx_projects_legacy_unique on public.projects(company_id,legacy_id) where legacy_id is not null;
create table public.project_members (
  project_id uuid not null references public.projects(id) on delete cascade,
  employee_id uuid not null references public.employees(id) on delete cascade,
  is_responsible boolean not null default false, primary key(project_id,employee_id)
);
create index idx_project_members_employee on public.project_members(employee_id,project_id);

create function public.current_employee_id() returns uuid language sql stable security definer set search_path=''
as $$ select e.id from public.employees e where e.auth_user_id=auth.uid() and e.status='aktiv' limit 1 $$;
create function public.current_employee_company_id() returns uuid language sql stable security definer set search_path=''
as $$ select e.company_id from public.employees e where e.auth_user_id=auth.uid() and e.status='aktiv' limit 1 $$;
create function public.current_employee_role() returns text language sql stable security definer set search_path=''
...
grant execute on function public.current_employee_id(), public.current_employee_company_id(), public.current_employee_role() to authenticated;

create table public.tasks (
  id uuid primary key default gen_random_uuid(), legacy_id text,
  company_id uuid not null default public.current_employee_company_id() references public.companies(id) on delete restrict,
  project_id uuid references public.projects(id) on delete set null,
  titel text not null check(length(btrim(titel))>0), beschreibung text, faellig date,
...
create trigger trg_tasks_updated_at before update on public.tasks for each row execute function public.tasks_set_updated_at();
create trigger trg_tasks_tenant_refs before insert or update on public.tasks for each row execute function public.tasks_enforce_tenant_refs();
...
revoke all on public.companies,public.profiles,public.roles,public.permissions,public.role_permissions,public.employees,public.projects,public.project_members,public.tasks from anon,authenticated;
grant select on public.companies,public.profiles,public.roles,public.permissions,public.role_permissions,public.employees,public.projects,public.project_members to authenticated;
grant select,insert,update on public.tasks to authenticated;

create policy profiles_select_self on public.profiles for select to authenticated using(id=auth.uid());
create policy companies_select_own on public.companies for select to authenticated using(id=public.current_employee_company_id());
...
create policy employees_select_company on public.employees for select to authenticated using(company_id=public.current_employee_company_id());
create policy projects_select_scoped on public.projects for select to authenticated using(
 company_id=public.current_employee_company_id() and (public.current_employee_role()='geschaeftsfuehrer' or exists(
  select 1 from public.project_members pm where pm.project_id=projects.id and pm.employee_id=public.current_employee_id())));
...
-- GF darf eigene gelöschte Tasks selektieren, damit UPDATE/Restore technisch möglich ist.
-- Die normale Repository-Liste filtert deleted_at weiterhin explizit auf NULL.
-- Bauleiter/Mitarbeiter sehen ausschließlich aktive, für sie gescopte Tasks.
create policy tasks_select_scoped on public.tasks for select to authenticated using(
 company_id=public.current_employee_company_id() and (
  public.current_employee_role()='geschaeftsfuehrer' or (
   deleted_at is null and (
    zugeordnet_employee_id=public.current_employee_id() or (project_id is not null and exists(
     select 1 from public.project_members pm where pm.project_id=tasks.project_id and pm.employee_id=public.current_employee_id()))
   )
  )
 ));

create policy tasks_insert_scoped on public.tasks for insert to authenticated with check(
 company_id=public.current_employee_company_id() and public.current_employee_role() in ('bauleiter','geschaeftsfuehrer') and (
  project_id is null or public.current_employee_role()='geschaeftsfuehrer' or exists(
   select 1 from public.project_members pm where pm.project_id=tasks.project_id and pm.employee_id=public.current_employee_id())));

create policy tasks_update_scoped on public.tasks for update to authenticated using(
 company_id=public.current_employee_company_id() and (
  public.current_employee_role()='geschaeftsfuehrer' or (
